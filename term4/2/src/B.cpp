...
private:
    ...
    void dfs2(size_t from)
    {
        ...
        for (auto i : edges[from]) {
            if (pipes[i].flow < pipes[i].capacity) {
                dfs2(pipes[i].to);
            }
        }
    }
...
public:
    Solution() : n(input()), m(input()), edges(n), shortestPaths(n), p(n), visited(n)
    {
        pipes.reserve(m * 2);
        ...
        for (size_t i = 0; i < m; ++i) {
            ...
            cin >> a >> b >> c;
            --a, --b;
            pipes.push_back({a, b, c, 0});
            pipes.push_back({b, a, c, 0});
